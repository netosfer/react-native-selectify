 ...
 * License: MIT License
 */
var formatDate = function(d, format=1) {
    var date = '' + d.getDate();
...
(function($) {
    ...
    $.fn.markyourcalendar = function(opts) {
        ...
        var days = {"Pazar": 0, "Pazartesi": 1, "Salı": 2, "Çarşamba": 3, "Perşembe": 4, "Cuma": 5, "Cumartesi": 6};
        // kuhanin ang mga pwedeng oras sa bawat araw ng kasalukuyang linggo
        this.getAvailableTimes = function() {
            var tmp = ``;
            for (i = 0; i < 7; i++) {
                var tmpAvailTimes = ``;
                $.each(settings.availability[days[getDayName(settings.startDate.addDays(i), 'tr-TR')]], function() {
                    var buttont = this;
                    var active_class = '';
                    var appointment_index = null;
                    console.log(getDayName(settings.startDate.addDays(i), 'tr-TR'))
                    $.each(settings.notAvailability, function(index) {
                        if(this.appointment_date === formatDate(settings.startDate.addDays(i)).toString() && buttont.toString() === this.appointment_time && !this.closed){
                            active_class = ' active'
                            appointment_index = index;
                        } else if(this.appointment_date === formatDate(settings.startDate.addDays(i)).toString() && buttont.toString() === this.appointment_time && this.closed){
                            active_class = ' disabled';
                        }
                    })
                    tmpAvailTimes += `
                        <a href="javascript:;" class="myc-available-time${active_class}" data-time="` + this + `" data-date="` + formatDate(settings.startDate.addDays(i)) + `" data-index="` + appointment_index + `">
                            ` + this + `
                        </a>
                    `;
                });

...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('settings.module_title') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ MainHelper::admin_url() }}">{{ __('global.dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('settings.module_title') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{{ __('settings.module_title') }}</h3>
                            <div class="card-tools">
                                <a class="btn btn-success fw-bold" href="{{ route('settings.add') }}"><i class="fa-duotone fa-grid-2-plus"></i> {{ __('settings.add_setting') }}</a>
                            </div>
                        </div>

                        <div class="card-body table-responsive">
                            <table class="table table-bordered setting_datatable">
                                <thead>
                                <tr>
                                    <th width="20">
                                    ...
                                    <th>{{ __('settings.site_title') }}</th>
                                    <th width="140px">Action</th>
                                </tr>
                                </thead>
                                <tbody></tbody>
                            </table>
                            <button class="btn btn-danger btn-sm bulk-delete-button d-none" type="button" data-post="{{ route('settings.bulk_delete')  }}" token="{{ csrf_token() }}"><i class="fa-duotone fa-trash-xmark"></i>Bulk Delete</button>
                        </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(function () {
            var table = $('.setting_datatable').DataTable({
                processing: true,
                serverSide: true,

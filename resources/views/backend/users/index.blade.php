...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('users.module_title') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ MainHelper::admin_url() }}">{{ __('global.dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('users.module_title') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{{ __('users.module_title') }}</h3>
                            <div class="card-tools">
                                <a class="btn btn-success fw-bold" href="{{ route('users.add') }}"><i class="icon-plus"></i> {{ __('users.add_kuser') }}</a>
                            </div>
                        </div>

                        <div class="card-body table-responsive">
                            <table class="table table-bordered kuser_datatable">
                                <thead>
                                <tr>
                                    <th width="20">
                                    ...
                                    <th width="80" style="max-width: 80px" class="more-dots">{{ __('users.profile_photo_path') }}</th>
									<th>{{ __('users.name') }}</th>
                            ...
                            <button class="btn btn-danger btn-sm bulk-delete-button d-none" type="button" data-post="{{ route('users.bulk_delete')  }}" token="{{ csrf_token() }}"><i class="fa-duotone fa-trash-xmark"></i>Bulk Delete</button>
                        </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(function () {
            var table = $('.kuser_datatable').DataTable({
                processing: true,
                serverSide: true,
